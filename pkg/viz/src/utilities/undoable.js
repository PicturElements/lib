...
class UndoableToggle extends Undoable {
	...
	do(target, key) {
		if (target == null)
			throw new TypeError(
				"Invalid toggle target: target must be an object or a function"
			);
	...
	toggle() {
		...
		if (typeof target.toggle == "function") target.toggle.call(target, key);
		else if (typeof target == "function") target.call(target, key);
		else {
...
class UndoableQueue extends UndoableArray {
	...
	push(item) {
		if (item instanceof Undoable) super.push(item);

		return this;
	}

	undo(idx) {
		if (idx instanceof Undoable) idx = this.indexOf(idx);
...
class UndoableDict extends Undoable {
	...
	get(key) {
		return hasOwn(this.dict, key) ? this.dict[key] : null;
	}

	setMulti(dict) {
		if (!dict) return;

		for (let k in dict) this.set(k, dict[k]);

		return this;
	}

	undo(key) {
		if (hasOwn(this.dict, key)) {
	...
	undoAll() {
		for (let k in this.dict) {
			if (hasOwn(this.dict, k)) this.undo(k);
		}
	...
	forEach(callback) {
		...
		for (let k in this.dict) {
			if (hasOwn(this.dict, k)) callback(this.dict[k], k, this);
		}
...
class UndoableWrapper extends Undoable {
	...
	set(store) {
		if (store instanceof Array) this.store = new UndoableQueue(...store);
	...
	undo() {
		if (this.store) this.store.undoAll();

		return this;
	}

	forEach(callback) {
		if (typeof callback != "function" || !this.store) return this;
...
function undoableFactory(type, doFunc, undoFunc, args) {
	...
	if (constr) return new constr(...args);
	else {
		if (typeof undoFunc != "function") {
			if (typeof doFunc == "function") {
				undoFunc = doFunc;
				doFunc = null;
			} else return console.warn("Cannot construct custom Undoable");
		}

		const undoable = new Undoable("generic");
		if (doFunc) undoable.do = wrapFunc(doFunc);
